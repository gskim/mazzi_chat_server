...
export const sendVerificationEmail = async (email: string, key: string) => {
	const emailSubject = `Hello! please verify your email`
	const emailBody = `Verify your email by clicking <a href="http://mazzi.com/users/verification/${key}">here</a>`
	return await sendEmail(emailSubject, emailBody, email)
}
